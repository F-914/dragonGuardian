...
function CalibrateScaleSprite:init()
    ...
    self.trophyAmount_ = OutGameData
        :getUserInfo()
        :getTrophyAmount()
    self.rewardNodeData_ = OutGameData
        :getUserInfo()
        :getUserInfoLadder()
        :getLadderList()
    ...
    for i = 1, #self.rewardNodeData_ do
        local num = self.rewardNodeData_[i].trophyCondition_
...
function CalibrateScaleSprite:update(dt)
    --监听到用户的钥匙数量发生变化
    if not self.trophyAmount_ ~= OutGameData.trophyAmount_ then
        self.trophyAmount_ = OutGameData.trophyAmount_
