...
local EventManger=require("src/app/manager/EventManager.lua")
...
function LineupLayer:init(lineupList)
    ...
    for i = 1, table.getn(lineupList) do
        local button=ccui.Button:create(ConstDef.ICON_LINEUP_LIST[lineupList[i]],ConstDef.ICON_LINEUP_LIST[lineupList[i]])
        layout:add(button)
