...
local TowerButtonLayer_2nd=require("src/app/ui/layer/TowerButtonLayer_2nd.lua")
...
function TowerDetialLayer_2nd:ctor(card,bag)
...
function TowerDetialLayer:init(card,bag)
   
    local ttf={}
    ttf.fontFilePath=PATH_TTF_BIAOZJW
    ttf.fontSize=30
    local mask=display.newColorLayer(cc.c4b(0, 0,0,360))
    --local mask=display.newSprite(PATH_MASK)
    self:add(mask)
    ...
    local icon_=self:createIcon(card,popup)
    ...
    skillIntroductionLabel:setPosition(0,baseSkillIntroducion:getContentSize().height-skillIntroduction:getContentSize().height)
    ...
    ratityLabel:setPosition(baseSkillIntroducion:getContentSize().width*0.6,baseSkillIntroducion:getContentSize().height*1.55)
    ...
    ttfAtkSpeed:setPosition(defaultLayoutAtkSpeed:getContentSize().width*0.3,defaultLayoutAtkSpeed:getContentSize().height*0.35)
    ...
    ttfTarget:setPosition(defaultLayoutTarget:getContentSize().width*0.3,defaultLayoutTarget:getContentSize().height*0.35)
    ...
    ttfSkillSlow:setPosition(defaultLayoutSkillSlow:getContentSize().width*0.3,defaultLayoutSkillSlow:getContentSize().height*0.35)
    ...
    ttfSkillTriggerTime:setPosition(defaultLayoutSkillTriggerTime:getContentSize().width*0.3,defaultLayoutSkillTriggerTime:getContentSize().height*0.35)
    ...
    enhanceAtk:setString("+"..card.atkEnhance)
   ...
   local ttfYellow={}
   ttfYellow.fontFilePath=PATH_TTF_HZGBJW
...
--[[--
 ...
 function TowerDetialLayer:createIcon(card,parents)
    local icon=TowerButtonLayer_2nd.new(card)
    parents:add(icon)
 ...
 function TowerDetialLayer:createLayout(parents,base,icon,texture,label)
    local ttf={}
    ttf.fontFilePath=PATH_TTF_BIAOZJW
    ttf.fontSize=30
