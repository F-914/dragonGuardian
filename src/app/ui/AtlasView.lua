...
--local AnimationLayer=require("src/app/ui/layer/AnimationLayer.lua")
local TowerDetialLayer=require("src/app/ui/layer/TowerDetialLayer_2nd.lua")
...
local PopupLayer=require("src/app/ui/layer/PopupLayer_2nd.lua")
...
local function createCheckbox(parents,number)
    local checkbox = ccui.CheckBox:create(PATH_ICON_UNCHOOSE, PATH_ICON_CHOOSE, PATH_ICON_CHOOSE, PATH_ICON_UNCHOOSE, PATH_ICON_UNCHOOSE)
...
function AtlasView:ctor()
    local function clickListener()--注册每0.1秒监听的函数，用于检测点击时间
        ...
        if table.getn(ConstDef.BUTTON_CLICK)==0 then
           ...
           for i=1,table.getn(BagList_) do--将图鉴中的塔设置为不可点击
    ...
    EventManager:regListener(EventDef.ID.RESUME_BAG_BUTTON,self,function ()--注册恢复图鉴中的塔为可点击
        for i=1,table.getn(BagList_) do
...
function AtlasView:createBag()
    ...
    if table.getn(ConstDef.COLLECTED)==0 then
        heightCollect=1
    elseif table.getn(ConstDef.COLLECTED)%4==0 then
          heightCollect=table.getn(ConstDef.COLLECTED)/4
       elseif table.getn(ConstDef.COLLECTED)%4~=0 then
          heightCollect=math.floor(table.getn(ConstDef.COLLECTED)/4)+1
       end
     
       if table.getn(ConstDef.UNCOLLECTED)==0 then
        heightUncollect=1
       elseif table.getn(ConstDef.UNCOLLECTED)%4==0 then
        heightUncollect=table.getn(ConstDef.UNCOLLECTED)/4
     elseif table.getn(ConstDef.UNCOLLECTED)%4~=0 then
        heightUncollect=math.floor(table.getn(ConstDef.UNCOLLECTED)/4)+1
     end
