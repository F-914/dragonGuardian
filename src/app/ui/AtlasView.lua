...
local function createCheckbox(parents, number)
    local checkbox = ccui.CheckBox:create(StringDef.PATH_ICON_UNCHOOSE, StringDef.PATH_ICON_CHOOSE,
        StringDef.PATH_ICON_CHOOSE, StringDef.PATH_ICON_UNCHOOSE, StringDef.PATH_ICON_UNCHOOSE)
...
function AtlasView:createLineupList()
    local lineupLayout = display.newSprite(StringDef.PATH_BASEMAP_AREA) --------------------------------------------------------------------
...
function AtlasView:createBag()
    ...
    local splitLineCollected = display.newSprite(StringDef.PATH_SPLITLINE_COLLECTED)
    ...
    splitLineCollected:setPosition(layoutTexture:getContentSize().width * 0.5,
        layoutTexture:getContentSize().height * 0
        - 50)

    local heightUncollect, heightCollect
    if #(ConstDef.COLLECTED) % 4 == 0 then
        heightCollect = #(ConstDef.COLLECTED) / 4
    elseif #(ConstDef.COLLECTED) % 4 ~= 0 then
        heightCollect = #(ConstDef.COLLECTED) / 4 + 1
    end

    if #(ConstDef.UNCOLLECTED) % 4 == 0 then
        heightUncollect = #(ConstDef.UNCOLLECTED) / 4
    elseif #(ConstDef.UNCOLLECTED) % 4 ~= 0 then
        heightUncollect = #(ConstDef.UNCOLLECTED) / 4 + 1
    end
    ...
    local splitLineUncollected = display.newSprite(StringDef.PATH_SPLITLINE_UNCOLLECTED)
    ...
    splitLineUncollected:setPosition(layoutUncollect:getContentSize().width * 0.5,
        layoutUncollect:getContentSize().height * 0.8)
