...
function MainScene:registerPanel()
    ...
    --registerLayer:setBackGroundColor(cc.c3b(51, 51, 51))
    --registerLayer:setBackGroundColorType(ccui.LayoutBackGroundColorType.solid)--设置颜色
    ...
    --输入框
    local editTxt = ccui.EditBox:create(cc.size(300,50),nil,50)
    ...
    --注册按钮
    local registerButton = ccui.Button:create("register.png","register.png")
    registerButton:setScale(1,1)
    --registerButton:setTitleText("Register")
    --registerButton:setTitleFontSize(50)
    ...
    registerButton:addTouchEventListener(function(sender,eventType)
        if eventType == ccui.TouchEventType.ended then
            local uid = getUUID()
            --os.execute("E:/planesfight/file/uid.txt")
...
function MainScene:mainPanel()
    ...
    --menuLayer:setBackGroundColor(cc.c3b(100, 0, 0))--设置为红色
    menuLayer:setBackGroundColorOpacity(180)--设置为透明
    --menuLayer:setBackGroundColorType(1)
    ...
    newGameButton:addTouchEventListener(function(sender,eventType)
        if eventType == ccui.TouchEventType.ended then--场景跳转
            ...
            local fightScene = import("app.scenes.FightingScene"):new()
            display.replaceScene(fightScene,"turnOffTiles",0.5)
            print(transform)
        end
    end)
